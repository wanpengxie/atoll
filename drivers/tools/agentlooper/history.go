...
func validateHistory(history []json.RawMessage) error {
	...
	for i, raw := range history {
		...
		if json.Unmarshal(raw, &head) != nil { return fmt.Errorf("message %d is not an object", i) }
		if len(pending) > 0 {
		...
		switch head.Role {
		case "toolResult": return fmt.Errorf("message %d has no pending tool call", i)
		case "assistant":
			calls, _, err := assistantParts(raw)
			if err != nil { return fmt.Errorf("message %d: %w", i, err) }
			pending = calls
		case "user":
		default: return fmt.Errorf("message %d has unsupported role", i)
		}
	}
	if len(pending) > 0 { return fmt.Errorf("history has %d unclosed tool calls", len(pending)) }
	return nil
}
...
func stopReason(raw json.RawMessage) string {
	var m struct { Reason string `json:"stopReason"` }
