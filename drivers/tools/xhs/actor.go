	...
	for {
		...
		case reaperTickType:
			...
			if _, err := a.sys.After(a.reaperInterval, reaperTickType, nil); err != nil {
				return fmt.Errorf("xhs: reaper re-arm failed, dying to re-forge: %w", err)
			}
		default:
			a.handle(msg)
		}
...
func (a *Actor) tryBind() error {
	...
	if err := a.dev.start(); err != nil {
		...
		if _, aerr := a.sys.After(a.bindRetryInterval, bindRetryType, nil); aerr != nil {
			return fmt.Errorf("xhs: bind-retry re-arm failed, dying to re-forge: %w", aerr)
		}
		return nil
	}
	// Bound: arm the first reaper tick; each tick re-arms the next in run().
	if _, err := a.sys.After(a.reaperInterval, reaperTickType, nil); err != nil {
		return fmt.Errorf("xhs: reaper arm failed, dying to re-forge: %w", err)
	}
	return nil
}
