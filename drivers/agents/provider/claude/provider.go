...
func (p provider) Spec() driverproto.ProviderSpec {
	return driverproto.ProviderSpec{Name: Class, Capabilities: driverproto.Capabilities{Steer: true, Interrupt: true, Resume: true}, Describe: introspect.Describe{Description: "Claude Code workspace agent backed by a dedicated local CLI process.", SkillDoc: AgentSkillDoc}, Selections: append([]driverproto.TurnOptions(nil), p.cfg.Selections...), DefaultSelection: p.cfg.Default}
}
