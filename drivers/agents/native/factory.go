...
func Manifest() introspect.Manifest {
	...
	ask.Description = "Submit work to an explicitly selected view_id. related_work_id continues that visible work's existing view. Missing scope is rejected; main/fork routing is an outer organization concern. Receipt mode requires submission_key."
	ask.ErrorCodes = append(ask.ErrorCodes, "scope_required", "view_not_found")
	ask.Examples = []json.RawMessage{json.RawMessage(`{"text":"explain the failure","view_id":"view:topic"}`)}
	...
	steer.ErrorCodes = append(steer.ErrorCodes, "scope_required", "view_not_found", "busy", "cas_mismatch", "target_gone")
	...
	for word, description := range map[string]string{
		...
		schema := `{"type":"object","properties":{"view_id":{"type":"string"},"work_id":{"type":"string"},"target":{"type":"string"},"old_text":{"type":"string"},"new_text":{"type":"string"},"duration_ms":{"type":"integer","minimum":1,"maximum":1800000}},"additionalProperties":false}`
		...
		m.Words[word] = introspect.WordSpec{Description: description, InputSchema: json.RawMessage(schema), OutputSchema: json.RawMessage(workapi.ControlOutputSchema), ErrorCodes: []string{"invalid_args", "scope_required", "view_not_found", "work_not_found", "cas_mismatch", "target_not_owned", "busy"}}
	}
	m.Capabilities["view_control"] = true
	return m
