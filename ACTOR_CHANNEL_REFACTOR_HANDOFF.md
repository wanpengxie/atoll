...
- Relationship uniqueness is checked from ordinary member definitions at Home introduction and declaration-change boundaries under one Home mutex, including aliases and concurrent calls. Genesis validates duplicate relations before insertion. No second persistent relationship table and no kind-based runtime singleton rule.
- Each new channel publishes separate peer and seat declarations. Recipe handles receive a parent-host overlay; no handle is synthesized from svc_agent. c0 control peer and parent relation are composed separately, merging the group/c0 case.
