...
// special-casing), so this is痛感-driven: a real file/system-event producer
// brings its core-type entry back with it.
//
// `system.heartbeat` is wire-only — impl-vocabulary §2.7 + proto-foundation
// §1.12 + proto-layer0 §3.1 specify it as a transport keepalive frame, NOT a
...
// per-step progress bubbles. impl-vocabulary §2.3 collapses that into
// `agent.text` + visibility=system, so no separate core-type registration.
//
