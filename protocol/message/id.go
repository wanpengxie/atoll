...
// Audience is the envelope's optional immediate-wakeup list. Each entry is a
// channel-local actor id; an event may use [] for a pure-log write. The wildcard
// `"*"` is removed from the closed set — fanout must enumerate explicit actor
// ids. Self-scheduling includes the sender's own actor id.
