...
// parent_id/correlation (single-shot, no closure). Authorization is membrane-uniform
// (PM-D1): read/write via channel-owner root ∪ active membership, delete additionally
// distinguishes the creator (PM-D3), create via channel membership (no object exists
// yet) — by-identity day-1; NO presented-cap field (deferred). There is no per-object
// grant operand: the membrane is one trust phase, so R does not exist.
type Invocation struct {
	...
	// the verdict; it holds no authorization state, makes no local decision, caches nothing
	...
	// value, read's selector); delete has NO Args operand (delete is by-id). RAW []byte
	// (opaque bytes), NOT json.RawMessage
...
// side trips the test.
var contentFields = []string{"caller", "resource", "operation", "args"}
