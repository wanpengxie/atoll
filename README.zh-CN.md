...
Atoll 之于 agent，就像 Linux 之于程序。你把它装在一台机器上；从此任意多个 agent
（codex、claude、你自己写的）、工具（MCP server、设备）和人，就在上面并排跑、住在共享的
房间里、服从同一套规则——并且每一个都免费拿到操作系统本该给一个进程的东西：一个身份、
一个住处、一条和别人说话的通道、权限、一份"发生过什么"的持久记录，以及一个"以后叫醒我"的办法。

| Unix | Atoll | |
|---|---|---|
| 进程 | **actor** | 一切主体：人、agent、工具、系统本身——同一种身份 |
| 文件系统 + 管道 | **channel（频道）** | 协作发生的地方，同时是**权威账本**：发生的一切都是频道有序日志里的一条消息 |
| 字节流 | **message（消息）** | 唯一的交互载体：请求 / 事件 / 响应；操作系统不解释内容 |
| 文件 | **resource（资源）** | 客体：文件、KV、外部对象；控制面在操作系统里，数据面委托出去 |
| 权限位 | **access（访问）** | 谁能进哪个频道、能做什么——由结构（成员身份）决定，不由某段程序决定 |
| cron | **timer（定时器）** | 一个"以后叫醒某个 actor"的持久承诺；重启不丢 |

"**一切皆消息，一切进账**"是 Atoll 的 everything-is-a-file。人的一句话、agent 的一次工具
调用、系统铸造一个成员、一次审批——同一种东西，追加在每个频道那一本有序账上。审计、回放、
恢复、可观测不是加上去的功能，是账本的推论。形状刻意贴近 *Android + Slack*：每个居民有
身份和沙箱，居民之间只能靠消息说话，房间是共享的。

> **状态：v0.01 —— pre-release，不对数据负责。**
> Atoll 今天已经能端到端跑起来（`atoll up` + web UI），但它是早期软件，不是产品。
> **存储格式、API 和 wire 协议会随时改，没有 deprecation 周期、没有迁移路径**——新版本可能
> 拒绝打开、或直接覆盖旧版本建的 node home。不要往里放任何你无法重建的东西。
> 见 [今天能做什么](#今天能做什么) 与 [状态](#状态)。
...
过去两年 agent 领域造出了大量优秀的"应用"——Claude Code、Codex、各家 harness、工作流
框架。它们都长在裸机上：底下没有操作系统。

- 每个产品自带一套会话、权限、工具、记忆、调度，互不相通；换模型就要换整套。
- 多个 agent、多个人一起干活时没有共同的真相——谁做了什么、谁有权做什么、任务推进到哪，
  散在各自的对话记录里。
- 长任务活不过一次上下文压缩；出了事没有账可查、没有统一的开关可关、没有费用的闸门。
- 社区在用同一批土办法补同一批洞——spec 文件、goal 判定器、loop、状态图——它们是同一个
  缺失物的四个侧面：什么必须成立、什么算完谁判、状态住哪、谁决定下一步。

这正是 1970 年前后应用软件面对的处境。Unix 的答案不是更强的应用，而是一个极小的核心、
几个不解释内容的原语、其余一切放在用户态。Atoll 把这个答案搬到 agent 上——并且要回答一个
Unix 没遇到过的问题：**当"进程"是会判断的智能体时，操作系统该组织什么？** 它的答案是：
操作系统组织的是共享记录之上的义务与注意力，而不是指令——但这层组织层（相当于 Erlang 之上
的 OTP）是接下来要造的东西，不是今天交付的东西。见 [docs/architecture](docs/architecture/README.md)。

## 今天能做什么

看快速开始之前先看这张表；这是诚实的那条线。

| | 今天（2026-08） |
|---|---|
| 一条命令装好并跑起一个节点 | ✅ `scripts/install.sh` → `atoll up`；root 账号、`c0`、本地 device |
| 一个 coding agent 坐在频道里 | ✅ codex 或 claude CLI 作为 `c0` 的 **steward** 跑在你的设备上；问它，它答 |
| 多个频道、多个 agent、多个人 | ✅ 频道树、`agent`/`tool` 模板、一个模板多个实例、账号 |
| 运行时挂 MCP server | ✅ 两条消息，不重编；它的工具变成消息类型 |
| 接入另一台机器 | ✅ `atoll-daemon` 以 device 身份加入 |
| 浏览器客户端 | ✅ [`atoll-web`](https://github.com/wanpengxie/atoll-web) 走公开契约（登录 / ws 帧 / obs）——它必须跟着节点的 wire 方言走，而方言在动 |
| 一切可脚本化 | ✅ bearer token、`/obs` 读、`/ws` 写——UI 能做的脚本都能做 |
| agent 在自己的模型循环里使用 Atoll 的工具（`call_actor` 等） | 🚧 工具口已有；codex / claude worker 还没把它暴露给模型 |
| 给 agent 程序化注入 prompt（我是谁、在哪个频道、谁在场） | 🚧 已设计，未建 |
...
这些是你在上面搭东西时可以依赖的性质。它们是结构性的——由代码的形状强制，不靠 prompt——而且
分层由机器检查（`archtest/`）；这一点重要，因为这个操作系统上的一部分代码会是跑在它上面的
agent 写的。

1. **频道账本即真相；可重放、可恢复、可 fork。** 每条消息先追加，然后别人才能看到；没有旁路，
   没有 REST "send"。actor 死掉、换模型、换机器，都从账恢复到同一状态。
2. **身份比在任者长寿。** actor 的 id 稳定；底下的模型进程可以重启、升级、换代（今天 codex、
   明天 claude），频道里还是同一个成员。任何 provider 只要把用量与步骤作为事件写进账、遵守停止
   协议就能接入——模型中立、harness 中立。
3. **权限住在膜上，不住在 prompt 里。** 谁能进频道、进来能说哪类话，由门在结构上判定、均匀作用
   于整个频道：没有按对象的 ACL，没有 token 冒充。agent 做的事在法律上就是它背后 principal
   做的事。"agent 越权"从防不胜防的运行时问题变成结构上不可达。
4. **声明式收敛；缺席恒不销毁。** 只有一种机制：账本上的期望状态减去宿主的证词 → 拉起或退役。
   没有级联 kill，没有隐藏定时器；崩溃、断网、重启之后系统收敛回账本说的样子。
5. **设备即算力，agent 天然有手。** 本机、容器、云端沙盒以 device 身份加入；agent 部署在上面
   就有 shell、文件系统、git、浏览器，不需要为每种工具再造适配层。存储与算力在接口后面，可整体
   换成云基础设施。
6. **组织层可安装，操作系统零改动。** 长任务治理（目标 / 认领 / 谁拍板 / 证据 / 是否闭合）、
   消息改道（审计、限流、计费、审批、A/B）、额度——每一样都是一份协议加一个服务成员，像
   Slack "add app to channel" 一样往频道里加。这就是 OTP 之于 Erlang 的那一层；已设计，未交付。
7. **人是一等成员。** 频道是一个人和 agent 在同一本账上说话、拍板、交接的房间。审批是发给某个人
   的一条消息，不是流程外的按钮。web UI 只是这套契约的一个客户端；任何 app、浏览器、IDE 插件
   都可以是另一个。
8. **自观测、同法自管理、可安全自举。** 系统对自己的观测与管理不走任何特权通道：`c0` 用同一套
   规则看着并管理所有频道；运维本身是一个有人有 agent 的频道；下一版在一个 fork 出来的 Atoll
   里造出、测过、再切换。

为什么长成这样——以及它和 harness、框架、MCP、A2A、Kubernetes、Erlang/OTP 的关系——是
[架构系列](docs/architecture/README.md)的主题。
...
Atoll 是 **v0.01 —— pre-release**。操作系统的核心——身份、频道、账本、成员身份、device、
定时器——已实现并强制；一条命令装好节点；coding agent 作为成员坐在频道里；web 客户端走公开契约
跟它对话。项目现在正在用自己开发自己：owner 和 agent 在一个频道里，agent 写码、互审，人来 merge。

这个版本号在实践中意味着：

- **不对数据负责。** 1.0 之前没有升级或迁移路径。SQLite schema、注册表、磁盘 home、wire 方言都会变；
  新二进制可能拒绝旧 home，或者悄悄把它重刻一遍。把 `~/.atoll` 当一次性的——安装器之所以提供
  "把旧 home 挪开"正是为此。
- **API 变动没有 deprecation 周期。** 消息类型、帧形状、Go 包表面都随系统评审修订；没有任何东西
  冻结。web 客户端和节点一起动；对不上会表现为 `type_unsupported` 或被拒的帧，不会静默出错。
- **已知缺口**见 [今天能做什么](#今天能做什么)：agent 还不能在自己的模型循环里看到 Atoll 的工具、
  还没有 prompt 层、组织层只有设计。
- **当前边界：** 一个节点一个信任域；`atoll up` 只替你管本地那一台 device（其它手动挂）。

先核心，再组织层，再打磨。想看后续就 watch 这个仓库。
