...
// WaitForAdmission applies daemon-side backpressure before a new message
// append can enqueue another view-sync outbox row.
func (o *ViewSyncOutbox) WaitForAdmission(ctx context.Context) error {
	if err := EnsureChannelTables(ctx, o.db); err != nil {
		return err
	}
	blocking := false
	for {
...
// EnqueueAppendTx inserts the view_sync_outbox row for an appended message
// inside the caller's messages transaction.
func (o *ViewSyncOutbox) EnqueueAppendTx(ctx context.Context, tx *sql.Tx, env *message.Envelope, seq int64) error {
	if err := ensureChannelTablesTx(ctx, tx); err != nil {
		return err
	}
	envJSON, err := json.Marshal(env)
	if err != nil {
		return fmt.Errorf("store: outbox encode: %w", err)
	}
	const q = `INSERT INTO view_sync_outbox
	   (seq, message_id, envelope_json, enqueued_at, status)
	   VALUES (?, ?, ?, ?, 'pending')`
	if _, err := tx.ExecContext(ctx, q, seq, env.ID, string(envJSON), env.TSReceived); err != nil {
