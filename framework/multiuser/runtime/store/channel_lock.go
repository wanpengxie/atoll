...
func (l *ChannelLock) Get(ctx context.Context) (ChannelLockRow, bool, error) {
	if err := EnsureChannelTables(ctx, l.db); err != nil {
		return ChannelLockRow{}, false, err
	}
...
func (l *ChannelLock) ValidateWriteTx(
	...
	if errors.Is(err, sql.ErrNoRows) {
		return &basestore.FencingStaleError{Reason: "channel_lock row missing"}
	}
	if err != nil {
	...
	return validateRow(row, true, placement.FencingToken(fencingToken), placement.DaemonEpoch(daemonEpoch))
}
...
func validateRow(
	...
	if row.FencingToken != fencingToken {
		return &basestore.FencingStaleError{
			HaveToken: fencing.FencingToken(row.FencingToken),
			GotToken:  fencing.FencingToken(fencingToken),
			HaveEpoch: fencing.DaemonEpoch(row.DaemonEpoch),
			GotEpoch:  fencing.DaemonEpoch(daemonEpoch),
