...
// token.
func awaitToken(t *testing.T, ch *platform.Home, pen harness.Pen, target actor.ActorID, reject string, overall time.Duration) string {
	t.Helper()
...
func TestRestartDaemonPlacedActor_RebuildsAcrossWire(t *testing.T) {
	...
	// The daemon: a REAL platform.RunCompute reconcile ring over a real wire to
