...
// newSweepingHome assembles a home.Home whose closure reconciler sweeps on a
// fast ticker, so the AUTOMATIC level backstop (not a manually-driven sweep) can
// be observed within a test.
func newSweepingHome(t *testing.T, every time.Duration) *home.Home {
	t.Helper()
...
func terminalExists(t *testing.T, ch *home.Home, parentID message.ID) bool {
	t.Helper()
...
func countTerminals(t *testing.T, ch *home.Home, parentID message.ID) int {
	t.Helper()
	rows, err := ch.View().ReadAfterSeq(context.Background(), 0, 1000)
	if err != nil {
		t.Fatalf("ReadAfterSeq: %v", err)
...
func TestReconciler_DeregEdgeRace_NoDuplicateTerminal(t *testing.T) {
	...
	for i := 0; i < authors; i++ {
		...
		go func(edge bool) {
			...
			for r := 0; r < rounds; r++ {
				if edge {
					home.DriveDownEdgeForTest(ch, workerID)
				} else {
					home.ReconcileClosureForTest(ch)
				}
			}
		}(edge)
	}
