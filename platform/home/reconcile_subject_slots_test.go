...
type subjectSlotAuthorityStub struct {
	list   func(context.Context) ([]storespec.ActorControlRow, error)
	lookup func(context.Context, actor.ActorID) (storespec.ActorControlRow, bool, error)
}

func (s subjectSlotAuthorityStub) ListActive(ctx context.Context) ([]storespec.ActorControlRow, error) {
	return s.list(ctx)
}
...
func TestSweepSubjectSlotsConvergesBothDirections(t *testing.T) {
	...
	authority := subjectSlotAuthorityStub{
		...
		lookup: func(_ context.Context, id actor.ActorID) (storespec.ActorControlRow, bool, error) {
			if id == "human:stale" {
	...
	authority := subjectSlotAuthorityStub{
		...
		lookup: func(_ context.Context, id actor.ActorID) (storespec.ActorControlRow, bool, error) {
			if id == "human:uncertain" {
				return storespec.ActorControlRow{}, false, errors.New("lookup unavailable")
			}
			return storespec.ActorControlRow{}, false, nil
		},
	}
...
func TestSweepSubjectSlotsTerminalReadProtectsConcurrentAdmit(t *testing.T) {
	...
	authority := subjectSlotAuthorityStub{
		...
		lookup: func(_ context.Context, got actor.ActorID) (storespec.ActorControlRow, bool, error) {
			if got != id {
			...
			slots.EnsureSlot(id)
			return storespec.ActorControlRow{ID: id, Kind: actor.KindHuman}, true, nil
		},
	}
