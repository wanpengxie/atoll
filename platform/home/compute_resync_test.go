...
type multiBuilder map[actor.ActorID]platform.ActorFactory

func (b multiBuilder) Lookup(id actor.ActorID) (platform.ActorFactory, bool) {
	f, ok := b[id]
	return f, ok
}
...
func runComputeAsync(t *testing.T, cfg compute.Config) {
	t.Helper()
	ctx, cancel := context.WithCancel(context.Background())
	done := make(chan struct{})
	go func() {
		defer close(done)
		_ = compute.Run(ctx, cfg)
	}()
...
func TestComputeReconcile_PureShrink_DeregistersHostRowSameTick(t *testing.T) {
	...
	runComputeAsync(t, compute.Config{
