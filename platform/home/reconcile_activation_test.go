...
type testBuilder struct {
	...
	byClass map[string]platform.ActorFactory
...
// recordFactory returns a platform.ActorFactory that records the caps it received and
...
// caps bundle (platform.ActorFactory's doc).
func (b *testBuilder) recordFactory(id actor.ActorID) platform.ActorFactory {
...
func (b *testBuilder) Lookup(id actor.ActorID) (platform.ActorFactory, bool) {
	b.mu.Lock()
	defer b.mu.Unlock()
	if fixture, ok := b.aliases[id]; ok {
		id = fixture
...
func (b *testBuilder) LookupByClass(childID actor.ActorID, class string, config json.RawMessage) (platform.ActorFactory, bool) {
	b.mu.Lock()
	defer b.mu.Unlock()
