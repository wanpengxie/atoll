		...
		case ctrlAttach:
			...
			lc.submitControlTask(func() {
				...
				if attachErr := a.canAttach(reqCtx, daemonID); attachErr != nil {
					...
					record.report(SessionAdmissionRejected, "attach_policy_rejected", attachErr)
					return
				}
				if _, activateErr := a.sessions.activate(record); activateErr != nil {
					record.report(SessionProtocolViolation, "duplicate_or_late_attach", activateErr)
					return
				}
				reply.Accepted = true
				raw, _ := encodeControl(controlFrame{
				...
				if sendErr := lc.sendControl(raw); sendErr != nil {
					record.report(SessionSpineLost, "accepted_reply_write_failed", sendErr)
					return
				}
				go a.probeSession(record, lc)
			}, func() {
				reply := AttachReply{
					ChannelID: a.channelID, DaemonID: daemonID,
					Generation: record.generation,
				...
				_ = lc.sendControl(raw)
				record.report(SessionAdmissionRejected, "attach_task_pool_busy", nil)
			})
		case ctrlPlanPull:
			frame, err := decodeControl(payload)
			...
			lc.submitControlTask(func() {
				reply := PlanReply{}
				if !authority.admits() {
					reply.Error = "link: plan pull outside active session"
				} else {
					var planErr error
					reply.Actors, planErr = a.plan(reqCtx, daemonID)
					if planErr != nil {
						reply.Error = planErr.Error()
					}
				}
				raw, _ := encodeControl(controlFrame{
				...
				reply := PlanReply{Error: "link: control task pool busy"}
				raw, _ := encodeControl(controlFrame{
					RequestID: requestID, Kind: ctrlPlanReply, PlanReply: &reply,
				})
				_ = lc.sendControl(raw)
