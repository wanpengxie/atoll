...
func TestLogbookRecentFiltersBeforeLimitAndOrdersAscending(t *testing.T) {
	...
	sys := &fakeSys{}
	New(Deps{Logbook: fakeLogbook{rows}}).handle(sys, requestMsg("q", registry.TypeLogbookRecent, []byte(`{"limit":5}`)))
...
func TestLogbookRecentExcludesCallerAndProtocolTraffic(t *testing.T) {
	rows := []storespec.StoredRow{logRow(1, message.KindRequest, "caller", "user.text"), logRow(2, message.KindRequest, "other", registry.TypeLogbookRecent), logRow(3, message.KindResponse, "system", registry.TypeLogbookRecent), logRow(4, message.KindResponse, "other", "answer")}
	sys := &fakeSys{}
	New(Deps{Logbook: fakeLogbook{rows}}).handle(sys, requestMsg("q", registry.TypeLogbookRecent, []byte(`{"limit":5}`)))
	got := sys.replies[0].v.(LogbookRecentResponse).Messages
...
func TestDescribeIncludesLogbookRecent(t *testing.T) {
	if _, ok := systemDescribe().Types[registry.TypeLogbookRecent]; !ok {
