...
// statusRequest builds an actor.status request envelope (empty payload = full
// self-snapshot). actor.status is not a kimi.command, so it does not go through
// the command() helper.
func statusRequest(id string) *message.Envelope {
	return &message.Envelope{
		ID:         message.ID(id),
		ChannelID:  testChannelID,
		Sender:     message.Sender{Kind: actor.KindAgent, ID: "agent:main"},
...
//  7. Status (online): with a device attached, actor.status answers
//     device_online=true.
func TestStatusOnline(t *testing.T) {
	...
	if err := a.Receive(context.Background(), statusRequest("req-st-on")); err != nil {
		t.Fatalf("Receive: %v", err)
	}
...
//  8. Status (offline): with no device, actor.status answers device_online=false
//     and still completes (it is a self-answer, never a device round-trip).
func TestStatusOffline(t *testing.T) {
	w := &recordingWriter{}
	a := startActor(t, w, Config{})

	if err := a.Receive(context.Background(), statusRequest("req-st-off")); err != nil {
		t.Fatalf("Receive: %v", err)
	}
