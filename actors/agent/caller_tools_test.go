...
func TestAwaitResultResolvesFinal(t *testing.T) {
	b := &Bridge{}
	caller := b.caller()
	caller.futures.Register("req-await", true)
...
func TestAwaitResultTimeoutReturnsStillPendingAck(t *testing.T) {
	b := &Bridge{}
	caller := b.caller()
	caller.futures.Register("req-slow-await", true)
...
func TestAbandonThenFinalBecomesTrigger(t *testing.T) {
	b := &Bridge{}
	caller := b.caller()
	caller.futures.Register("req-aband-tool", true)
...
func TestListPendingReturnsIdListOnly(t *testing.T) {
	b := &Bridge{}
	caller := b.caller()
	caller.futures.Register("lp-1", true)
	caller.futures.Register("lp-2", true)
