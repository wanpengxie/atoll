...
// the poll/wake loop from spinning (§3.2 钉块; Linux timerfd/epoll 校准 —
// block until the nearest deadline, never a for{sleep} poll). A production
// Clock wraps time.Now/time.NewTimer directly; a fake Clock (schedule_test.go)
// lets the vertical-slice tests advance time deterministically — zero sleep,
// zero wall-clock flake, the time-axis analogue of actorrt.Config.Clock
// (discipline② 确定性可测).
//
...
// harness's Advance() landing in exactly that window, §3.2 钉7's own
// discipline taken one step further) would silently arm for the WRONG
// instant. An absolute deadline has no such conversion to go stale.
type Clock interface {
	...
	// Stop() it before arming a replacement (§3.2 钉7: Go time.Timer
...
// library. OpenScheduler (the runtime-root assembly seam, §3.4) fills this in
// when the downstream-supplied AssemblyDeps.Clock is nil; the engine's own
// New stays fail-fast on a nil Clock (v1.2 codex-minor: 两层各守各的默认).
