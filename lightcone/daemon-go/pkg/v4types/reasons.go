...
// HTTP status table for daemon_rpc binding.
...
// these via the binding-specific transport (daemon_rpc HTTP → L2 §3.6.1
// status table; in_worker_bus → Result.Err per L2 §3.6.2).
...
// HTTPStatus returns the HTTP status code the daemon_rpc HTTP binding
// MUST emit when rejecting with this reason. Per L2 §3.6.1 table.
//
...
// reject (HTTP for daemon_rpc, Result.Err for in_worker_bus).
...
// HTTPStatus returns the HTTP status the daemon_rpc HTTP binding MUST
// emit when an install API rejects with this reason. Per L2 §3.6.1
