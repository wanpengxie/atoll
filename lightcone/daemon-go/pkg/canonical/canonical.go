// Package canonical implements the RFC 8785 (JCS) canonical JSON
// serialization + SHA-256 (hex, lowercase) hash mandated by L2 §1.4.10.2
// for the v4 protocol baseline.
//
...
//     §3.2.2.3 (shortest round-trip; fixed for 1e-6 ≤ |x| < 1e21; scientific
//     otherwise with `e+N` / `e-N`, no leading zero in exponent).
...
//     integer payloads survive without float64 round-trip precision loss.
//
...
func CanonicalHash(e v4types.Envelope) (string, error) {
...
// CanonicalHashPayload returns SHA-256 (hex, lowercase) over the RFC 8785
// canonicalization of `payload`. `payload` MUST be valid JSON (object,
// array, or scalar). Used by adapter deterministic response id derivation
// per L2 §1.4.10.2.
...
func envelopeHashInput(e v4types.Envelope) (map[string]any, error) {
	// payload: parse raw JSON via UseNumber so integers retain precision.
	var payload any
	if len(e.Payload) == 0 {
		// L0 §2.2: payload non-null but {} legal. We refuse a totally
		// empty payload here because the caller's normalize pass should
		// have substituted `{}` already. Empty = caller bug.
		return nil, errors.New("canonical: envelope.payload is empty; normalize must run first")
	}
	v, err := decodeJSON(e.Payload)
	if err != nil {
...
// appendCanonicalString appends the JCS-quoted form of s to buf.
//
// Escape rules (RFC 8785 §3.2.2.2):
//   - 0x22 ("), 0x5C (\) → `\"`, `\\`
...
func appendCanonicalString(buf []byte, s string) []byte {
	...
	for i := 0; i < len(s); {
		...
		if r == utf8.RuneError && size == 1 {
			...
			// RFC 8785 expects valid UTF-8 input; we degrade gracefully
			// rather than panic.
...
// formatJSONNumber takes the raw JSON number string from
// encoding/json's `json.Number` and renders it per ECMAScript
// Number→String (RFC 8785 §3.2.2.3).
//
// Integer-shaped numbers (matching `^-?\d+$` and fitting int64) pass
// through unchanged — they are already in canonical form. Anything with
// a fractional or exponent component is re-rendered through formatFloatES.
func formatJSONNumber(n json.Number) (string, error) {
	...
	if isIntegerLiteral(s) {
		// Validate by ParseInt for non-leading-zero / range correctness.
		// ECMAScript ToString(int) drops the leading + and emits decimal.
		// For values exceeding int64 range we fall back to float to mimic
		// ES Number's IEEE 754 double semantics.
...
// formatFloatES renders f per ECMAScript Number→String (RFC 8785 §3.2.2.3).
//
// Rules:
//   - NaN / ±Inf are not representable in JSON (RFC 8785 forbids them).
//   - 0 (positive or negative) → "0".
//   - Other values: shortest round-trip decimal digits; fixed
//     representation iff -6 ≤ es-exp ≤ 20 (i.e. 1e-6 ≤ |x| < 1e21);
//     scientific (`e+N` / `e-N`, no leading zeros in exponent) otherwise.
func formatFloatES(f float64) (string, error) {
	...
	// Shortest round-trip in scientific form via strconv. Result like
	//   "1e+05" / "-1.5e-07" / "2.5e+00"
	s := strconv.FormatFloat(f, 'e', -1, 64)

	// Parse out sign / mantissa-digits / decimal-exponent.
	sign := ""
	idx := 0
	if s[0] == '-' {
		sign = "-"
	...
	// digits = all mantissa digits (with `.` removed).
	var digits string
	...
	// Strip insignificant trailing zeros from digits, adjusting exp.
	// (Go's 'e' format already produces shortest, but the parse-then-
	// re-render path should preserve that — strip is defensive.)
	for len(digits) > 1 && digits[len(digits)-1] == '0' {
		digits = digits[:len(digits)-1]
		exp++
	}

	// In strconv 'e' format the value equals
	//   (digits as int) * 10^(exp - (len(mantFrac)))
	// but the "exp" reported is for the form "D.frac e+E", so the
	// es-style exponent (= position of decimal point - 1 in integer-form
	// digits) is exactly `exp` (since strconv already aligns to the
	// first digit before the dot).
	esExp := exp
	...
	// Scientific output. RFC 8785 / ES: "Ne+M" or "N.NNNe+M" with no
	// leading zero in the exponent and lowercase `e`.
	mantOut := digits
	if len(digits) > 1 {
...
// renderFixed lays digits at decimal-position esExp+1 (i.e. there are
// esExp+1 digits before the implicit decimal point), padding with zeros
...
func renderFixed(digits string, esExp int) string {
	...
	if intLen >= len(digits) {
		// All-integer with trailing zeros.
