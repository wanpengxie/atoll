...
retired='GenerateCoord|placement_coord|ReserveCreate|CommitReservation|Committed|ReservationID|resource_reservations|resource_tombstones|TombstoneID|Scrubber|choosePlacement|is_dir|json:"dir"|\bDir[[:space:]]+(bool|\*)|\bDir:'
if rg -n "$retired" "${production[@]}" --glob '*.go' --glob '!**/*_test.go'; then
...
if rg -ni "$deferred" "${production[@]}" --glob '*.go' --glob '!**/*_test.go'; then
