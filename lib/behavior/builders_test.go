...
func TestBuildRequest(t *testing.T) {
	sender := message.Sender{Kind: "agent", ID: "agent:a"}
	env, err := BuildRequest("ch", sender, builderClock, RequestSpec{
		Type:     "x.y",
	...
	if _, err := BuildRequest("ch", sender, builderClock, RequestSpec{Audience: message.Audience{"a"}}); err == nil {
		t.Fatal("missing type: want error")
	}
	if _, err := BuildRequest("ch", sender, builderClock, RequestSpec{Type: "x"}); err == nil {
		t.Fatal("missing audience: want error")
	}

	// Caller id scheme override.
	env, _ = BuildRequest("ch", sender, builderClock, RequestSpec{
...
func TestBuildEvent(t *testing.T) {
	sender := message.Sender{Kind: "agent", ID: "agent:a"}
	env, err := BuildEvent("ch", sender, builderClock, EventSpec{
	...
	if _, err := BuildEvent("ch", sender, builderClock, EventSpec{}); err == nil {
		t.Fatal("missing type: want error")
	}
}
