...
func SystemWordSpecs() map[string]WordSpec {
	return map[string]WordSpec{
		...
		message.TypeSystemRequestCancel: systemWord(
			"Close an open request in this channel that you did not send. Closure authorship is a closed set — the receiver, the caller itself, or the substrate — so a third party has no way to write that terminal and asks the substrate to observe it instead. The request ends the same way a passed deadline ends it (unanswered_timeout), with the substrate as author and closed_by/requested_by recording who asked. To close a request you DID send, cancel it directly instead; to stop work already running, use agent.interrupt.",
			"request_id (string): the id of the open request to close.",
			objectSchema(`"request_id":{"type":"string"}`, "request_id")),
